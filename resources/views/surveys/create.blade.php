            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form id="survey-create-form" action="{{ route('surveys.store') }}" method="POST" x-data="surveyForm()" data-initial-fields="{{ e(json_encode(old('fields', [['type' => 'text', 'label' => '', 'name' => '', 'required' => false, 'optionsText' => '']]))) }}">
                    ...
                    <div class="mb-6">
                        ...
                        <template x-for="(field, i) in fields" :key="i">
                            <div class="border border-gray-200 rounded-md p-4 mb-3 bg-gray-50">
                                ...
                                <div class="flex items-center gap-2 mb-2">
                                    <input type="checkbox" x-model="field.required" :id="'req-' + i">
                                    <label :for="'req-' + i">Wajib diisi</label>
                                </div>
                        ...
                        <div x-show="fields.length === 0" class="text-gray-500 text-sm">Belum ada field. Klik "Tambah field".</div>
                    </div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700">Simpan Survey</button>
                    </div>
                </form>
            </div>
        </div>
